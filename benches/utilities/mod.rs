...
fn filter_shared_usize(bencher: Bencher) {
    let mut rng = rand::thread_rng();
    let die: Uniform<usize> = Uniform::from(0..=1000);
    let slice: Vec<usize> = die.sample_iter(&mut rng).take(1000).collect();
    let other: Vec<usize> = die.sample_iter(&mut rng).take(1000).collect();
